...
- Antigravity sends BMP, JPEG, PNG, and WebP images and common audio formats as native blocks,
  UTF-8 text as embedded resources, and PDFs as local resource links. Text is limited to 1 MiB
  per file, images to 10 MiB each, and all attachments to 50 MiB per turn. Unsupported formats
  or oversized inputs fail explicitly. The session advertises the ACP client file system
  capability, so workspace reads and writes come back through `fs/read_text_file` and
  `fs/write_text_file` and are confined to the workspace and attachments directory. Native path
  permissions still apply to PDFs.
