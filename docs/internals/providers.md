...
The seven built-in drivers are Codex, Claude, Cursor, Grok, Kimi, OpenCode, and Antigravity.
See [builtInDrivers.ts](../../apps/server/src/provider/builtInDrivers.ts).
...
Antigravity accepts BMP, JPEG, PNG, and WebP images, common audio formats, UTF-8 text, and
PDF resource links. Limits are 1 MiB per text file, 10 MiB per image, and 50 MiB per turn.
Unsupported or oversized inputs fail explicitly, and native PDF path permissions still apply.
OpenCode sends supported images, text, and PDFs up to 20 MB as native file parts; other files
fall back to their paths in the turn text.
