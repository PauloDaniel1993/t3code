...
export function renderStopWindowsInstallProcessesScript(executablePath: string): string {
  return [
    `$targetPath = ${powershellSingleQuote(executablePath)}`,
    "$deadline = [DateTime]::UtcNow.AddSeconds(15)",
    "$matchedProcess = $false",
    "do {",
    "  $processes = @(Get-CimInstance Win32_Process | Where-Object { [string]::Equals($_.ExecutablePath, $targetPath, [System.StringComparison]::OrdinalIgnoreCase) })",
    "  if ($processes.Count -eq 0) { break }",
    "  $matchedProcess = $true",
    "  $processes | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }",
    "  Start-Sleep -Milliseconds 200",
    "} while ([DateTime]::UtcNow -lt $deadline)",
    'if ($processes.Count -gt 0) { throw "Timed out stopping desktop processes for $targetPath" }',
    "if ($matchedProcess) { Start-Sleep -Milliseconds 1000 }",
  ].join("\n");
}
...
async function writeWindowsShortcutFiles(installDir: string): Promise<void> {
  ...
  const pinnedTaskbarDir = NodePath.join(
    appData,
    "Microsoft",
    "Internet Explorer",
    "Quick Launch",
  ...
  const pinnedShortcutPath = NodePath.join(pinnedTaskbarDir, WINDOWS_LOCAL_SHORTCUT_NAME);
  const legacyPinnedShortcutPaths = WINDOWS_LEGACY_LOCAL_SHORTCUT_NAMES.map((name) =>
    NodePath.join(pinnedTaskbarDir, name),
  );
  const hasLocalPin =
    (await pathExists(pinnedShortcutPath)) ||
    (await Promise.all(legacyPinnedShortcutPaths.map(pathExists))).some(Boolean);
  if (hasLocalPin) {
    await Promise.all(legacyPinnedShortcutPaths.map((path) => NodeFSP.rm(path, { force: true })));
    await writeWindowsShortcut({
      ...shortcut,
      shortcutPath: pinnedShortcutPath,
    });
  }
}
...
async function findLaunchTarget(
  ...
  if (platform === "linux") {
    for (const candidate of ["t3code-alpha-local", "t3code", "AppRun"]) {
...
async function stopRunningWindowsInstall(installDir: string): Promise<void> {
  if (readCliHostPlatform() !== "win32" || !(await pathExists(installDir))) {
    return;
  }
  const executablePath = await findLaunchTarget(installDir, "win");
  if (!executablePath) {
  ...
  process.stdout.write(`[install-desktop] Stopping running desktop build at ${executablePath}\n`);
  await runCommand(
    "powershell.exe",
    [
      "-NoProfile",
      ...
      "Bypass",
      "-Command",
      renderStopWindowsInstallProcessesScript(executablePath),
    ],
    REPO_ROOT,
  );
}
...
export async function installDesktopBuild(options: InstallDesktopBuildOptions): Promise<void> {
  ...
  try {
    if (options.platform === "win") {
      await stopRunningWindowsInstall(options.installDir);
    }
