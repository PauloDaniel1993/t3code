...
import {
  ...
  renderStopWindowsInstallProcessesScript,
...
it("renders a Windows launcher with local state and taskbar identity", () => {
  ...
  assert.include(launcher, 'set "T3CODE_DESKTOP_DISPLAY_NAME=T3 alpha.local"');
...
it("renders exact-path Windows process termination for updates", () => {
  const script = renderStopWindowsInstallProcessesScript(
    "C:\\Users\\O'Brien\\AppData\\Local\\T3 Code Local\\T3 alpha.local.exe",
  );

  assert.include(
    script,
    "$targetPath = 'C:\\Users\\O''Brien\\AppData\\Local\\T3 Code Local\\T3 alpha.local.exe'",
  );
  assert.include(script, "[System.StringComparison]::OrdinalIgnoreCase");
  assert.include(script, "Stop-Process -Id $_.ProcessId -Force");
  assert.include(script, "$deadline = [DateTime]::UtcNow.AddSeconds(15)");
  assert.include(script, "if ($processes.Count -gt 0)");
  assert.include(script, "Start-Sleep -Milliseconds 1000");
});
...
it("renders a Windows shortcut script targeting the local launcher with the installed exe icon", () => {
  const script = renderWindowsShortcutScript({
    shortcutPath:
      "C:\\Users\\alice\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\T3 alpha.local.lnk",
  ...
  assert.include(
    script,
    "$shortcut.TargetPath = 'C:\\Users\\alice\\AppData\\Local\\T3 alpha.local\\T3 alpha.local.cmd'",
  );
  assert.include(script, "$shortcut.Arguments = ''");
  assert.include(
    script,
    "$shortcut.WorkingDirectory = 'C:\\Users\\alice\\AppData\\Local\\T3 alpha.local'",
  );
  assert.include(
    script,
...
it("renders install metadata for direct taskbar launches", () => {
  ...
  assert.equal(metadata.displayName, "T3 alpha.local");
