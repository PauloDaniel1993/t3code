...
function UnavailableTaskComposer(props: {
  ...
  return (
    <View className="bg-screen px-4 pt-1.5" style={{ paddingBottom: (props.bottomInset ?? 0) + 6 }}>
      <View className="w-full self-center" style={{ maxWidth: props.contentMaxWidth }}>
        <View
          accessibilityLabel={`Composer unavailable. ${props.reason}`}
          accessibilityRole="text"
          className="gap-1 rounded-2xl bg-card px-4 py-[13px]"
        >
          <Text className="text-xs font-t3-bold leading-[18px] text-foreground">
            Composer unavailable
          </Text>
          {/* This card is opaque, and this branch applies no ancestor opacity
              multiplier to the mandatory refusal reason. */}
          <Text className="text-2xs leading-[17px] text-foreground-muted">{props.reason}</Text>
        </View>
      </View>
    </View>
  );
