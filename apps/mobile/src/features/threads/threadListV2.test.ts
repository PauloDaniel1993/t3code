...
describe("buildThreadListV2ListItems", () => {
  ...
  it("keeps unsent pending tasks distinct from persisted nested task threads", () => {
    const parent = makeThread({ id: ThreadId.make("parent"), title: "Parent" });
    const child = makeThread({
      id: ThreadId.make("child"),
    ...
    const parentOnly = buildThreadListV2Items({
      threads: [parent, child],
      nestedTaskThreadKeys: new Set([`${environmentId}:${child.id}`]),
      environmentId: null,
      searchQuery: "",
    ...
    const items = buildThreadListV2ListItems({
      items: parentOnly.items,
      pendingTasks: [makePendingTask("child")],
    });

    expect(items.map((item) => [item.type, item.key])).toEqual([
      ["v2-thread", `v2-thread:${environmentId}:parent`],
      ["v2-pending", "v2-pending-task:child"],
    ]);
  });
