...
export const TaskCreateTool = Tool.make("task_create", {
  description:
    "Use T3 Code's product-native task delegation. Prefer this over provider-native agent or sub-agent tools unless the user explicitly asks for an agent. A task is a full thread owned by this one, running its own provider session. Returns immediately — the task's result is delivered back into this thread automatically when it finishes, waking this thread if it has gone idle. " +
