...
> {
  return (
    ...
    event.type === "thread.task-created" ||
    event.type === "thread.task-updated" ||
    event.type === "thread.task-finished" ||
...
function adaptThreadDetailEventForSubscriber(
  ...
  const projectedEvent =
    event.type === "thread.activity-upserted"
      ? projectThreadActivityEvent(event)
      : projectActivityEvent(event, options.reasoningMessages);
  if (options.activityUpserts || projectedEvent.type !== "thread.activity-upserted") {
...
function projectThreadActivityEvent(event: OrchestrationEvent): OrchestrationEvent {
  if (event.type !== "thread.activity-appended" && event.type !== "thread.activity-upserted") {
    return event;
  }
  return {
    ...event,
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const dispatchFromClient = (
        ...
        options?: OrchestrationEngine.OrchestrationDispatchOptions,
      ) =>
        orchestrationEngine.dispatch(
          command,
          hasClientOrigin || options !== undefined
            ? {
        ...
        Effect.gen(function* () {
          ...
          const bootstrapProgram = Effect.gen(function* () {
            ...
            const started = yield* Effect.uninterruptible(
              dispatchFromClient(finalTurnStartCommand, options),
            );
          ...
          observeRpcEffect(
            ...
            Effect.gen(function* () {
              ...
              const shouldStopSessionAfterCommand = archiveCommand
                ...
                : false;
              const result = yield* dispatchNormalizedCommand(normalized).pipe(
        ...
        [ORCHESTRATION_WS_METHODS.replayEvents]: (input) =>
          observeRpcEffect(
            ...
            Stream.runCollect(
              ...
              Effect.map((events) => Array.from(events, projectThreadActivityEvent)),
              Effect.mapError(
                (cause) =>
          ...
          observeRpcStreamEffect(
            ...
            Effect.gen(function* () {
              ...
              const liveStream = orchestrationEngine.streamDomainEvents.pipe(
                ...
                Stream.map((event) => ({
                  kind: "event" as const,
                  event: adaptThreadDetailEventForSubscriber(event, {
                    activityUpserts: input.activityUpserts === true,
            ...
            projectCloneTracker.start(input, {
              createProject: (project) =>
                Effect.gen(function* () {
                  ...
                  yield* recordClientCommandAnalytics(normalizedCommand.command);
          ...
          observeRpcEffect(
            ...
            Effect.gen(function* () {
              ...
              if (input.resource._tag === "attachment") {
                const attachmentResource = input.resource;
                if (
                  parseThreadSegmentFromAttachmentId(attachmentResource.attachmentId) ===
                  PENDING_ATTACHMENT_THREAD_SEGMENT
                ) {
                  return yield* issueAssetUrl({ resource: attachmentResource });
                }
                if (attachmentResource.threadId === undefined) {
                  return yield* new AssetAttachmentNotFoundError({ resource: attachmentResource });
                }
                const attachment = yield* projectionSnapshotQuery
                  .getThreadAttachmentById({
                    threadId: attachmentResource.threadId,
                    attachmentId: attachmentResource.attachmentId,
                  })
                  .pipe(
                    Effect.mapError(
                      (cause) =>
                ...
                return yield* issueAssetUrl({
                  resource: {
                    ...
                    ...(attachmentResource.disposition !== undefined
                      ? { disposition: attachmentResource.disposition }
                      : {}),
                  },
                });
              }
