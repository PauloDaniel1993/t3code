...
describe("normalizeDispatchCommand attachments", () => {
  it.effect("accepts 100 inline images and rejects 101 before writing files", () =>
    Effect.gen(function* () {
      ...
      if (accepted.command.type !== "thread.turn.start") throw new Error("Wrong command");
      expect(accepted.command.message.attachments).toHaveLength(100);
      expect(NodeFS.readdirSync(config.attachmentsDir)).toEqual([".staging"]);
      yield* accepted.attachmentStage?.abort ?? Effect.void;
  ...
  it.effect("rejects decoded image overflow before writing it and removes earlier files", () =>
    Effect.gen(function* () {
      ...
      let writtenBytes = 0;
      const dataUrl = `data:image/png;base64,${Buffer.alloc(5 * 1024 * 1024).toString("base64")}`;
      const command = turnStartCommand({
        attachments: [
          ...Array.from({ length: 16 }, () => ({ dataUrl, sizeBytes: 5 * 1024 * 1024 })),
...
describe("question attachments", () => {
  it.effect("enforces the total response limit and claims duplicate filenames independently", () =>
    Effect.gen(function* () {
      ...
      if (normalized.command.type !== "thread.user-input.respond" || !normalized.attachmentStage) {
        throw new Error("Wrong command");
      }
      yield* normalized.attachmentStage.claim;
      yield* normalized.attachmentStage.commit;
      const attachments = Object.values(normalized.command.attachmentsByQuestionId!).flat();
      ...
      expect(
        NodeFS.readdirSync(config.attachmentsDir).filter((entry) => entry !== ".staging"),
      ).toEqual([`${id}.txt`]);
    }).pipe(Effect.provide(testLayer)),
  );
  ...
  it.effect("preserves a __proto__ question key and cleans up its claimed files", () =>
    Effect.gen(function* () {
      ...
      yield* normalized.attachmentStage.claim;
      yield* normalized.attachmentStage.commit;
      expect(Object.keys(normalized.command.attachmentsByQuestionId!)).toEqual(["__proto__"]);
      const attachment = normalized.command.attachmentsByQuestionId!["__proto__"]![0]!;
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const normalized = yield* normalizeDispatchCommand(command);
        if (
          normalized.command.type !== "thread.user-input.respond" ||
        ...
        yield* normalized.attachmentStage.claim;
        yield* normalized.attachmentStage.commit;
        expect(normalized.command.answers).toEqual(command.answers);
        const image = normalized.command.attachmentsByQuestionId!.q1![0]!;
        const file = normalized.command.attachmentsByQuestionId!.q2![0]!;
        expect(
        ...
        expect(retry.command.type).toBe("thread.user-input.respond");
        yield* retry.attachmentStage?.abort ?? Effect.void;
      }).pipe(Effect.provide(testLayer)),
  );
