...
type TaskProgressRuntimeEvent = Extract<ProviderRuntimeEvent, { type: "task.progress" }>;
...
function taskProgressHasActivityState(payload: TaskProgressRuntimeEvent["payload"]): boolean {
  return (
...
function toScheduledInput(
  ...
  const baseReplacementKey = providerEventReplacementKey(input.event);
  // Upstream models task activity and typed usage as independent latest-state
  // streams. A combined event is left uncoalesced; a usage-only event gets a
  // distinct key so the fork scheduler cannot replace one stream with the
  // other while a lane is under pressure.
  const replacementKey =
    input.event.type === "task.progress" && input.event.payload.typedUsage !== undefined
      ? taskProgressHasActivityState(input.event.payload)
        ? undefined
        : baseReplacementKey === undefined
          ? undefined
          : `${baseReplacementKey}:usage`
      : baseReplacementKey;
...
export function runtimeEventToActivities(
  ...
  switch (event.type) {
    ...
    case "task.progress": {
      ...
      const title =
        event.payload.description.trim().length > 0
          ...
          : {};
      const hasProgressState = taskProgressHasActivityState(event.payload);
      return [
        ...(hasProgressState
          ? [
              {
    ...
    case "tool.progress": {
      const isTaskHeartbeat = event.payload.taskId !== undefined;
      return [
        {
          // Agent-owned heartbeats stay bounded to one row per task. Parent
          // tool progress keeps the fork's provider-aware stable identity
          // when the adapter supplied enough canonical tool identity.
          id: isTaskHeartbeat
            ? EventId.make(`tool-progress:${event.threadId}:${event.payload.taskId}`)
            : (stableToolActivityId(event) ?? event.eventId),
          createdAt: event.createdAt,
          tone: isTaskHeartbeat ? "info" : "tool",
          kind: "tool.progress",
...
const make = Effect.gen(function* () {
  ...
  // Capture the two latest samples before the scheduler replaces usage events.
  // Compaction needs the transition, while client activity only needs the latest value.
  const contextUsageByProviderThread = yield* Cache.make<string, ReadonlyArray<number>>({
    capacity: 10_000,
    timeToLive: Duration.infinity,
    lookup: () => Effect.succeed([]),
  });
  const enrichCompactionUsage = Effect.fn("enrichCompactionUsage")(function* (
    event: ProviderRuntimeEvent,
  ) {
    if (
    ...
    const provider = event.providerInstanceId ?? event.provider;
    const key = `${provider.length}:${provider}${event.threadId}`;
    if (event.type === "thread.token-usage.updated" && event.payload.usage.usedTokens >= 0) {
      const samples = yield* Cache.get(contextUsageByProviderThread, key);
      yield* Cache.set(contextUsageByProviderThread, key, [
        ...samples.slice(-1),
        event.payload.usage.usedTokens,
      ]);
    } else if (event.type === "thread.state.changed" && event.payload.state === "compacted") {
      const samples = yield* Cache.get(contextUsageByProviderThread, key);
      yield* Cache.invalidate(contextUsageByProviderThread, key);
      const [beforeTokens, afterTokens] = samples;
      if (beforeTokens !== undefined && afterTokens !== undefined && afterTokens < beforeTokens) {
        return {
          ...event,
          payload: {
            ...event.payload,
            beforeTokens: event.payload.beforeTokens ?? beforeTokens,
            afterTokens: event.payload.afterTokens ?? afterTokens,
          },
        };
      }
    } else if (event.type === "session.started" || event.type === "session.exited") {
      yield* Cache.invalidate(contextUsageByProviderThread, key);
    }
    return event;
  });
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      yield* Effect.forEach(activities, (activity) =>
        Effect.gen(function* () {
          // Runtime event ids represent append-only history. Any derived id is
          // a latest-state slot and must preserve its original ordering while
          // replacing the payload, including upstream task usage/heartbeats.
          const isStableUpsert = activity.id !== event.eventId;
  ...
  const scheduler = yield* makeProviderIngestionScheduler(
    ...
    (input) => processInput(input).pipe(logIngestionFailure(input.source, input.event)),
  );
  ...
  const detectProviderDiffRepository = Effect.fn("detectProviderDiffRepository")(function* (
    ...
    yield* scheduler.enqueue(toScheduledInput({ source: "diff", event }));
  });
      ...
      yield* forkParked(
        Stream.runForEach(providerService.streamEvents, (event) =>
          event.type === "turn.diff.updated"
            ...
            : enrichCompactionUsage(event).pipe(
                Effect.flatMap((enrichedEvent) =>
                  scheduler.enqueue(toScheduledInput({ source: "runtime", event: enrichedEvent })),
                ),
              ),
        ),
      );
  ...
  return {
    start,
    // The diff worker feeds the per-thread scheduler, so drain it first.
    drain: diffWorker.drain.pipe(Effect.andThen(scheduler.drain)),
