...
projectionSnapshotLayer("ProjectionSnapshotQuery", (it) => {
    ...
    Effect.gen(function* () {
      ...
      yield* sql`
        ...
        VALUES (
          ...
          '[{"type":"image","id":"thread-1-image","name":"diagram.png","mimeType":"image/png","sizeBytes":5},{"type":"file","id":"thread-1-pdf-file","name":"reference.pdf","mimeType":"application/pdf","sizeBytes":7},{"type":"file","id":"thread-1-file","name":"notes.ts","mimeType":"text/plain","sizeBytes":9}]',
          0,
          '2026-02-24T00:00:04.000Z',
      ...
      const threadDetail = Option.map(threadDetailSnapshot, ({ thread }) => thread);
  ...
  it.effect("finds submitted-answer attachments with thread-scoped access", () =>
    Effect.gen(function* () {
      const snapshotQuery = yield* ProjectionSnapshotQuery;
      const sql = yield* SqlClient.SqlClient;
      const activeThreadId = ThreadId.make("thread-answer-attachments");
      const otherThreadId = ThreadId.make("thread-answer-attachments-other");
      const archivedThreadId = ThreadId.make("thread-answer-attachments-archived");
      const deletedThreadId = ThreadId.make("thread-answer-attachments-deleted");
      ...
      const deletedFile = {
        type: "file" as const,
        id: "deleted-answer-file",
        name: "deleted.txt",
        mimeType: "text/plain",
        sizeBytes: 7,
      };

      for (const [threadId, archivedAt, deletedAt] of [
        [activeThreadId, null, null],
        [otherThreadId, null, null],
        [archivedThreadId, createdAt, null],
        [deletedThreadId, null, createdAt],
      ] as const) {
        yield* sql`
          INSERT INTO projection_threads (
            thread_id,
      ...
      yield* sql`
        INSERT INTO projection_thread_messages (
          ...
          ${yield* encodeChatAttachments([messageFile])},
          0,
          ${createdAt},
          ${createdAt}
        )
      ...
      for (const [activityId, threadId, attachmentsByQuestionId] of [
        [
          "activity-answer-attachments",
          activeThreadId,
          { imageQuestion: [answerImage], fileQuestion: [answerFile] },
        ],
        ["activity-archived-answer-attachment", archivedThreadId, { question: [archivedFile] }],
        ["activity-deleted-answer-attachment", deletedThreadId, { question: [deletedFile] }],
        [
          "activity-invalid-answer-attachment",
          activeThreadId,
          {
            question: [
              {
                type: "file",
                id: "invalid-answer-file",
                name: "invalid.txt",
                mimeType: "text/plain",
                sizeBytes: 0,
              },
      ...
      assert.deepEqual(
        yield* snapshotQuery.getThreadAttachmentById({
          ...
          attachmentId: answerImage.id,
        }),
        Option.some(answerImage),
      );
      assert.deepEqual(
        yield* snapshotQuery.getThreadAttachmentById({
          threadId: activeThreadId,
          attachmentId: answerFile.id,
        }),
        Option.some(answerFile),
      );
      assert.deepEqual(
        yield* snapshotQuery.getThreadAttachmentById({
          threadId: activeThreadId,
          attachmentId: messageFile.id,
        }),
        Option.some(messageFile),
      );
      assert.deepEqual(
        yield* snapshotQuery.getThreadAttachmentById({
      ...
      assert.deepEqual(
        yield* snapshotQuery.getThreadAttachmentById({
          ...
          attachmentId: archivedFile.id,
        }),
        Option.some(archivedFile),
      );

      const invalidMetadataError = yield* snapshotQuery
        .getThreadAttachmentById({
          threadId: activeThreadId,
          attachmentId: "invalid-answer-file",
        })
        .pipe(Effect.flip);
      assert.equal(invalidMetadataError._tag, "PersistenceDecodeError");
    }).pipe(
      Effect.ensuring(
        Effect.gen(function* () {
          const sql = yield* SqlClient.SqlClient;
          yield* sql`
            ...
            WHERE thread_id LIKE 'thread-answer-attachments%'
          `;
          yield* sql`
  ...
  it.effect("preserves an unknown future attachment kind in thread history", () =>
    Effect.gen(function* () {
      const snapshotQuery = yield* ProjectionSnapshotQuery;
      const sql = yield* SqlClient.SqlClient;
      ...
      const result = yield* snapshotQuery.getThreadDetailSnapshot(threadId);
      assert.equal(result._tag, "Some");
      if (result._tag === "Some") {
        assert.deepEqual(result.value.thread.messages[0]?.attachments, [
          {
            type: "archive",
            id: "thread-future-attachment-1",
            name: "bundle.zip",
  ...
  it.effect("bounds initial activity snapshots and pages equal-order history without gaps", () =>
    Effect.gen(function* () {
      ...
      const readClientDetail = snapshotQuery
        .getThreadDetailSnapshot(threadId)
        .pipe(Effect.map(Option.map(({ thread }) => thread)));
      ...
      const detail = yield* readClientDetail;
      ...
      const nullBranchDetail = yield* readClientDetail;
      ...
      `;
      const fullWindowDetail = yield* readClientDetail;
      ...
      const largeDetail = yield* readClientDetail;
  ...
  it.effect("bounds activity hydration and preserves unresolved requests", () =>
    Effect.gen(function* () {
      ...
      if (fullDetail._tag === "Some") {
        // Internal detail readers retain upstream's wider bounded window. The
        // client snapshot path below owns the independent 200-row history page.
      ...
      if (
        ...
        const projectedFullBaseline = projectThreadDetailSnapshot({
          ...fullSnapshot.value,
          thread: {
            ...fullSnapshot.value.thread,
            activities: fullSnapshot.value.thread.activities.map(
              (activity) => rawActivitiesById.get(activity.id) ?? activity,
            ),
          },
        });
        assert.deepStrictEqual(projectedFullSnapshot, projectedFullBaseline);
        ...
        const failedCommand = projectedWindowSnapshot.thread.activities.find(
