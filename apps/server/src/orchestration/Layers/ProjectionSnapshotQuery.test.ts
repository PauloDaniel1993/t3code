  ...
  it.effect("bounds activity hydration and preserves unresolved requests", () =>
    Effect.gen(function* () {
      ...
      if (fullDetail._tag === "Some") {
        // Upstream bounds this read at THREAD_DETAIL_ACTIVITY_LIMIT (500); the fork
        // bounds it more tightly at PROVIDER_EVENT_FLOW_CONTROL.initialActivityPageSize
        // (200), which is the page size its cursor pagination contract is built on.
        // The fork reads pageSize + 1 rows and drops the probe row, so the newest 200
        // survive: activity-0302 through activity-0501.
        assert.equal(fullDetail.value.activities.length, 200);
        assert.equal(fullDetail.value.activities[0]?.id, asEventId("activity-0302"));
      ...
      if (detailWithPinnedRequests._tag === "Some") {
        ...
        // 200 windowed rows (see above) plus the three pinned unresolved requests.
        assert.equal(detailWithPinnedRequests.value.activities.length, 203);
