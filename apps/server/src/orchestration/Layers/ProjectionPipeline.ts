...
interface AttachmentSideEffects {
  ...
  readonly prunedThreadRelativePaths: Map<ThreadId, Set<string>>;
}

function addPrunableAttachmentPaths(
  sideEffects: AttachmentSideEffects,
  threadId: ThreadId,
  relativePaths: ReadonlySet<string>,
): void {
  const existing = sideEffects.prunedThreadRelativePaths.get(threadId);
  if (existing) {
    for (const relativePath of relativePaths) existing.add(relativePath);
    return;
  }
  sideEffects.prunedThreadRelativePaths.set(threadId, new Set(relativePaths));
}
...
function collectThreadActivityAttachmentRelativePaths(
  threadId: ThreadId,
  activities: ReadonlyArray<ProjectionThreadActivity>,
): Set<string> {
  const threadSegment = toCanonicalThreadAttachmentSegment(threadId);
  ...
  for (const activity of activities) {
    ...
    for (const attachment of Object.values(payload.value.attachmentsByQuestionId).flat()) {
      if (parseThreadSegmentFromAttachmentId(attachment.id) !== threadSegment) continue;
      const relativePath = attachmentRelativePath(attachment);
...
const recordAttachmentCleanupIntents = Effect.fn("recordAttachmentCleanupIntents")(
  function* (input: { readonly sideEffects: AttachmentSideEffects; readonly createdAt: string }) {
    ...
    for (const threadId of input.sideEffects.deletedThreadIds) {
      ...
      yield* attachmentCleanupQueue.enqueue({
    ...
    for (const [threadId, relativePaths] of input.sideEffects.prunedThreadRelativePaths) {
      ...
      for (const relativePath of relativePaths) {
        ...
        if (!attachmentId || parseThreadSegmentFromAttachmentId(attachmentId) !== threadSegment) {
          yield* Effect.logWarning("skipping unsafe projected attachment cleanup path", {
            threadId,
            relativePath,
          });
          continue;
  ...
  function* () {
      ...
      switch (event.type) {
        ...
        case "thread.message-sent": {
          ...
          if (previousMessage !== undefined && event.payload.attachments !== undefined) {
            const prunableRelativePaths = collectThreadAttachmentRelativePaths(
              event.payload.threadId,
              [previousMessage],
            );
            if (prunableRelativePaths.size > 0) {
              const currentRows = yield* projectionThreadMessageRepository.listByThreadId({
                threadId: event.payload.threadId,
              });
              for (const referencedRelativePath of collectThreadAttachmentRelativePaths(
                event.payload.threadId,
                currentRows,
              )) {
                prunableRelativePaths.delete(referencedRelativePath);
              }
              const currentActivities = yield* projectionThreadActivityRepository.listByThreadId({
                threadId: event.payload.threadId,
              });
              for (const referencedRelativePath of collectThreadActivityAttachmentRelativePaths(
                event.payload.threadId,
                currentActivities,
              )) {
                prunableRelativePaths.delete(referencedRelativePath);
              ...
              if (prunableRelativePaths.size > 0) {
                addPrunableAttachmentPaths(
                  attachmentSideEffects,
                  event.payload.threadId,
                  prunableRelativePaths,
                );
              }
            }
          }
        ...
        case "thread.reverted": {
          ...
          const prunableRelativePaths = collectThreadAttachmentRelativePaths(
            event.payload.threadId,
            existingRows.filter((message) => !keptMessageIds.has(message.messageId)),
          );
          for (const referencedRelativePath of collectThreadAttachmentRelativePaths(
            event.payload.threadId,
            keptRows,
    ...
    const applyThreadActivitiesProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        ...
        case "thread.reverted": {
          ...
          const keptActivityIds = new Set(keptRows.map((activity) => activity.activityId));
          const prunableRelativePaths = collectThreadActivityAttachmentRelativePaths(
            event.payload.threadId,
            existingRows.filter((activity) => !keptActivityIds.has(activity.activityId)),
          );
    ...
    const runProjectorForEvent = Effect.fn("runProjectorForEvent")(function* (
      ...
      const attachmentSideEffects: AttachmentSideEffects = {
        ...
        prunedThreadRelativePaths: new Map<ThreadId, Set<string>>(),
      };
    ...
    const projectEventDeferred: OrchestrationProjectionPipelineShape["projectEventDeferred"] =
      Effect.fn("projectEventDeferred")(
        function* (event) {
          ...
          // engine drains the queue only after its command transaction commits.
          // @effect-diagnostics-next-line returnEffectInGen:off
          return Effect.void;
        },
        Effect.provideService(AttachmentCleanupQueueRepository, attachmentCleanupQueue),
...
export const OrchestrationProjectionPipelineLive = Layer.effect(
  ...
  Layer.provideMerge(AttachmentCleanupQueueRepositoryLive),
