...
describe("OrchestrationEngine", () => {
  ...
  it("rejects handoff when the target provider model is not ready", async () => {
    let nextSequence = 8;
    const appendedEvents: OrchestrationEvent[] = [];
    const eventStore: OrchestrationEventStoreShape = {
      append: (event) =>
    ...
    const sourceThread: OrchestrationThread = {
      id: ThreadId.make("thread-handoff-source-not-ready"),
      projectId: asProjectId("project-handoff-ready"),
      title: "Source Thread",
      modelSelection: {
        instanceId: ProviderInstanceId.make("codex"),
        model: "gpt-5-codex",
    ...
    const projectionSnapshot = {
      ...
      projects: [
        {
          id: asProjectId("project-handoff-ready"),
          title: "Bootstrap Project",
          workspaceRoot: "/tmp/project-handoff-ready",
          defaultModelSelection: {
            instanceId: ProviderInstanceId.make("codex"),
            model: "gpt-5-codex",
          },
    ...
    const layer = OrchestrationEngineLive.pipe(
      ...
      Layer.provide(OrchestrationCommandReceiptRepositoryLive),
      Layer.provide(SqlitePersistenceMemory),
      Layer.provideMerge(makeProviderRegistryLayer([makeServerProvider({ status: "error" })])),
      Layer.provideMerge(NodeServices.layer),
    );
    ...
    await expect(
      runtime.runPromise(
        engine.dispatch({
          type: "thread.handoff.create",
          commandId: CommandId.make("cmd-handoff-to-not-ready-opus"),
          sourceThreadId: sourceThread.id,
          targetThreadId: ThreadId.make("thread-handoff-target-not-ready"),
          targetModelSelection: {
            instanceId: ProviderInstanceId.make("claude"),
            model: "claude-opus-4.8",
          },
    ...
    ).rejects.toThrow("is not ready and cannot receive handoff");
