...
async function createSettingsBackedOrchestrationSystem(settings: {
  ...
  readonly projectSettingsOverrides?: Record<
    string,
    {
      readonly threadTaskMaxRunning?: number;
      readonly threadTaskMaxTotal?: number | null;
...
describe("OrchestrationEngine", () => {
  ...
  it("uses the parent project's task cap in authoritative dispatch", async () => {
    const suffix = "project-scoped";
    const projectId = asProjectId(`project-task-limits-${suffix}`);
    const system = await createSettingsBackedOrchestrationSystem({
      threadTaskMaxTotal: 100,
      projectSettingsOverrides: {
        [projectId]: { threadTaskMaxTotal: 1 },
      },
    });
    const tasks = await initializeTaskParent(system, suffix);

    await system.run(tasks.createTask("1"));
    await system.run(tasks.finishTask("1"));

    const projectLimit = await system.run(Effect.exit(tasks.createTask("2")));
    expect(Exit.isFailure(projectLimit)).toBe(true);
    expect(JSON.stringify(projectLimit)).toContain("has created 1 tasks (limit 1)");
