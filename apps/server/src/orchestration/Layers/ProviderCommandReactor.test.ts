...
describe("ProviderCommandReactor", () => {
  ...
  it("awaits a provider turn id when replaying a durable pending request", async () => {
    const firstSend = Effect.runSync(Deferred.make<void>());
    const harness = await createHarness({
      sendTurnEffect: () => Deferred.succeed(firstSend, undefined).pipe(Effect.asVoid),
    });
    ...
    await Effect.runPromise(Deferred.await(firstSend));
    await harness.drain();
