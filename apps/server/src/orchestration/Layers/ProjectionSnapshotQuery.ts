...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const getThreadDetailByIdBounded = (threadId: ThreadId, bounds: ThreadDetailBounds | undefined) =>
    Effect.gen(function* () {
      ...
      // Activity-history pagination is the fallback for unwindowed clients.
      // Turn-window clients already receive a bounded, disjoint activity slice
      // and must not lose rows to the independent history-page limit.
      const pagesActivityHistory = bounds === undefined;
      const hasMoreActivities =
        pagesActivityHistory &&
        activityRows.length > PROVIDER_EVENT_FLOW_CONTROL.initialActivityPageSize;
      const displayedActivityRows = hasMoreActivities ? activityRows.slice(1) : activityRows;
      // Blocking request payloads must remain available even if they predate the
      // window the paging above kept. Pinned rows are merged into what renders
      // but are deliberately kept out of the cursor math: they are older than
      // the page, so letting one set `beforeCursor` would rewind pagination and
      // re-serve rows the client already holds.
      const selectedActivityRows = [
        ...new Map(
          [...displayedActivityRows, ...pinnedActivityRows].map(
      ...
      const thread = {
        ...
        ...(pagesActivityHistory
          ? { activityHistory: activityHistoryPageInfo(displayedActivityRows, hasMoreActivities) }
          : {}),
