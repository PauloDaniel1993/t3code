...
export interface BootServiceStep {
   ...
   * failures leave a state a later strict step either tolerates or fails
   * loudly on.
   */
  readonly optional?: boolean;
  /** Treat a non-zero process result as successful when this predicate allows it. */
  readonly ignoreFailure?: (result: ProcessRunner.ProcessRunOutput) => boolean;
...
const isLaunchdBootoutAbsent = (result: ProcessRunner.ProcessRunOutput): boolean => {
  ...
  return (
    /Could not find (?:specified )?service\b/i.test(output) ||
    /(?:^|\r?\n)\s*Service(?:\s+"[^"]+")?\s+(?:is )?not loaded\.?\s*$/im.test(output) ||
    /(?:^|\r?\n)\s*Boot-out failed:\s*3:\s*No such process\.?\s*$/im.test(output)
  );
};
...
export function launchdManager(input: {
  ...
  // A bootout may report an already-absent service, which is fine to proceed
  // from. Other failures stay strict. The strict `bootstrap` runs last and is
  // also the start:
    ...
    stop: [
      {
        ...
        ignoreFailure: isLaunchdBootoutAbsent,
        timeout: STOP_STEP_TIMEOUT,
      },
    ],
    activate: [
...
export const make = Effect.fn("cloud.boot_service.make")(function* (input: {
  ...
  const writeDurably = (filePath: string, contents: string) =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        // Opened read-write because Windows rejects FlushFileBuffers on a
        // read-only handle; POSIX fsync is happy either way.
        yield* (yield* fs.open(tempPath, { flag: "r+" })).sync;
        yield* fs.rename(tempPath, filePath);
        // Deliberately the REAL host, not `platform`: that one is the target
        // being configured for (systemd => "linux", even in tests running on
        // Windows), while fsync support depends on the OS we actually run on.
        ...
        // oxlint-disable-next-line t3code/no-global-process-runtime -- `platform` is the simulated service target in tests; fsync support depends on the real host.
  ...
  const runStep = Effect.fn("cloud.boot_service.run_step")(function* (
    ...
    return yield* runner.run({ command, args, timeout: options?.timeout }).pipe(
      ...
      Effect.filterOrFail(
        (result) => result.code === 0 || options?.ignoreFailure?.(result) === true,
        (result) =>
  ...
  const runSteps = (steps: ReadonlyArray<BootServiceStep>) =>
    Effect.forEach(
      ...
      (entry) => {
        const options =
          entry.timeout === undefined && entry.ignoreFailure === undefined
            ? undefined
            : {
                ...(entry.timeout === undefined ? {} : { timeout: entry.timeout }),
                ...(entry.ignoreFailure === undefined
                  ? {}
                  : { ignoreFailure: entry.ignoreFailure }),
              };
        const run = runStep(entry.step, entry.command, entry.args, options);
