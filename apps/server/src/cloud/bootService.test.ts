...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  const control: { failCommand: string | undefined; failureStderr: string } = {
  ...
  return { service, makeService, fs, path, statePath, commands, timeouts, control };
});
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect("installs, reports current state, and uninstalls on macOS", () =>
    Effect.gen(function* () {
      const { service, fs, path, statePath, commands, timeouts } = yield* makeHarness("darwin");
      const plan = yield* service.install;
  ...
  it.effect("propagates a fatal launchd bootout during repair", () =>
    Effect.gen(function* () {
      const { service, commands, control } = yield* makeHarness("darwin");
      yield* service.install;
      commands.length = 0;
  ...
  it.effect("does not remove the plist after a fatal launchd bootout during uninstall", () =>
    Effect.gen(function* () {
      const { service, fs, control } = yield* makeHarness("darwin");
      const plan = yield* service.install;
      control.failCommand = "launchctl bootout --wait gui/501/com.t3tools.t3code.service";
