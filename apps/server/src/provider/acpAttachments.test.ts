...
function writeAttachment(
  ...
  const filePath = NodePath.join(attachmentsDir, relativePath);
...
it.effect("maps mixed ACP attachments in order with image and resource-link shapes", () =>
  Effect.gen(function* () {
    ...
    const pdfPath = writeAttachment(attachmentsDir, pdf, Uint8Array.from([3, 4, 5]));
...
it.effect("skips unknown ACP attachment kinds", () =>
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem;
    ...
    const parts = yield* mapAcpAttachments({
      attachmentsDir,
