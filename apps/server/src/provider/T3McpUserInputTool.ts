...
When the \`t3-code\` MCP server exposes \`request_user_input\`, use that product-native structured user-input tool for asking up to ten questions in one call.

If both the \`t3-code\` MCP \`request_user_input\` tool and a provider-native or host-injected question tool are available, call the T3 MCP tool. The provider-native question tool is disabled while the T3 MCP user-input tool is available and will return an error.
`.trim();
