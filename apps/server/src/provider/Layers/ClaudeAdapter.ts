...
const buildUserMessageEffect = Effect.fn("buildUserMessageEffect")(function* (
  ...
  for (const attachment of input.attachments ?? []) {
    ...
    const fileType = attachment.type === "file" ? lookupAttachmentFileType(attachment.name) : null;
    const isPdf = attachment.type === "file" && attachment.mimeType === "application/pdf";
    const isInlineText =
      attachment.type === "file" &&
      fileType?.mimeType === attachment.mimeType &&
      fileType.contentKind === "text";
    if (attachment.type === "file" && !isPdf && !isInlineText) {
      continue;
    }
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const sendTurn: ClaudeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
    ...
    // and attachment reads must succeed before any turn state is mutated.
