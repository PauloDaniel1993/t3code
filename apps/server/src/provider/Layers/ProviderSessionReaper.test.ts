...
describe("ProviderSessionReaper", () => {
  ...
  async function startReaper(logCaptureLayer = Layer.empty) {
    await runtime!.runPromise(
      Effect.gen(function* () {
        const reaper = yield* ProviderSessionReaper;
        scope = yield* Scope.make("sequential");
        yield* reaper.start().pipe(Scope.provide(scope), Effect.provide(logCaptureLayer));
      }),
    );
  }
  ...
  it("reaps stale persisted sessions without active turns", async () => {
    ...
    await startReaper(harness.logCaptureLayer);
