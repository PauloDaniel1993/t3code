...
const makeMockGrokWrapper = Effect.fn("makeMockGrokWrapper")(function* (
  ...
  return yield* Effect.promise(async () => {
    ...
    const wrapperPath = NodePath.join(dir, windows ? "fake-grok.cmd" : "fake-grok.sh");
    const envExports = Object.entries(extraEnv ?? {})
      ...
      .join(windows ? "\r\n" : "\n");
    // JSON string literals are also valid shell words and keep Windows paths intact here.
    // @effect-diagnostics-next-line preferSchemaOverJson:off
    const quotedMockAgentCommand = JSON.stringify(mockAgentCommand);
    // @effect-diagnostics-next-line preferSchemaOverJson:off
    const quotedMockAgentPath = JSON.stringify(mockAgentPath);
    const script = windows
      ...
      : `#!/bin/sh
${envExports}
exec ${quotedMockAgentCommand} ${quotedMockAgentPath} "$@"
`;
...
it.layer(grokAdapterTestLayer)("GrokAdapterLive", (it) => {
  it.effect("sends runtime context with the current model without changing saved prompts", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* makeMockGrokWrapper({ T3_ACP_REQUEST_LOG_PATH: requestLogPath });
