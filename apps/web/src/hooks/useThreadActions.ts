...
function topOfPinnedRunOrderKey(): string | undefined {
  ...
  for (const shell of readThreadShells()) {
    // Task children render inside their parent's group and never participate
    // in the pinned run. Ignore stale child pin data from older clients when
    // choosing the placement key for a new top-level pin.
