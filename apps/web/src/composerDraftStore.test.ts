...
describe("composerDraftStore moveComposerPromptAndImages", () => {
  ...
  it("moves prompt and attachments to the destination without revoking preview URLs", () => {
    ...
    store.addAttachment(sourceDraftId, {
      type: "file",
      id: "file-move",
      name: "redirect.ts",
      mimeType: "text/typescript",
      sizeBytes: 6,
      file: new File(["export"], "redirect.ts", { type: "text/typescript" }),
    });
    ...
    expect(destination?.attachments.map((attachment) => attachment.id)).toEqual([
      "img-move",
      "file-move",
    ]);
