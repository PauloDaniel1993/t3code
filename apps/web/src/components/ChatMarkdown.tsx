...
function useChatMarkdownState({
  ...
  const markdownUrlTransform = useCallback((href: string) => {
    ...
    return (
      rewriteMarkdownFileHrefForRendering(href) ??
...
const CHAT_MARKDOWN_COMPONENTS = {
  ...
  img: function MarkdownImage({ node, title, src, alt, ...props }) {
    ...
    const fallbackCopyMarkdown =
      imageSource._tag === "WorkspaceFile" && !/^file:/i.test(classifiedSrc)
        ? copyMarkdown
        : undefined;
    // Keep ordinary authored paths copyable, but do not retain blocked schemes
    // or file URLs in DOM metadata when no thread can resolve their bytes.
    return (
      <ChatMarkdownImageFallback alt={altText} copyMarkdown={fallbackCopyMarkdown} kind={kind} />
    );
  },
