...
const assetUrlTestState = vi.hoisted(() => ({
...
vi.mock("../../assets/assetUrls", () => ({
  ...
  useAssetUrls: (_environmentId: unknown, resources: ReadonlyArray<unknown>) => {
    for (const resource of resources) {
      assetUrlTestState.resources.push(resource);
    }
    return resources.map(() =>
      assetUrlTestState.resolvesAssets ? "https://environment.test/api/assets/signed" : null,
    );
  },
  useAssetUrlState: (_environmentId: unknown, resource: unknown) => {
    assetUrlTestState.resources.push(resource);
    return resource === null || !assetUrlTestState.resolvesAssets
      ? { _tag: "Loading" as const }
      : { _tag: "Success" as const, url: "https://environment.test/api/assets/signed" };
  },
}));
...
beforeEach(() => {
  assetUrlTestState.resources.length = 0;
  assetUrlTestState.resolvesAssets = true;
});
...
describe("MessagesTimeline", () => {
  ...
  it("does not render the preview image before its URL resolves", () => {
    assetUrlTestState.resolvesAssets = false;
    const markup = renderToStaticMarkup(
  ...
  it("renders video attachments with the shared video player", () => {
    ...
    expect(assetUrlTestState.resources).toContainEqual({
      _tag: "attachment",
      attachmentId: "attachment-demo-mp4",
      threadId: "thread-1",
      fileName: "demo.mp4",
      mimeType: "video/mp4",
    });
