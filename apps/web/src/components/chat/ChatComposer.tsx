...
export function buildComposerContextAttachmentAssetRequest(
  source: ComposerContextClipboardFragment["source"],
  record: { readonly attachmentId: string },
) {
  const resource = {
    _tag: "attachment" as const,
    attachmentId: record.attachmentId,
  };
  if (record.attachmentId.startsWith("pending-")) {
    return { environmentId: source.environmentId, input: { resource } };
  }
  if (source.threadId === undefined) return null;
  return {
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const composerContextActions = useMemo(
    () => ({
      ...
      expandVideo: (fileId: string) => {
        ...
        const persistedPreview = buildAttachmentVideoPreview(
          environmentId,
  ...
  const runAttachmentImport = useCallback(
    async (
      ...
      const sourceConnection = readPreparedConnection(source.environmentId);
      if (!sourceConnection) {
        fail("The environment it came from is not connected.");
        return;
      }
      const assetRequest = buildComposerContextAttachmentAssetRequest(source, record);
      if (assetRequest === null) {
        fail("The original message did not identify its thread.");
        return;
      }
      const result = await createAssetUrl(assetRequest);
      const url =
        result._tag === "Success"
  ...
  const importAttachmentRecord = useCallback(
    async (
      ...
      try {
        await runAttachmentImport(record, localId, source, importTargetKey);
      } finally {
   ...
   * when `source` is given; the stash restores its own images and files.
   */
  const importContextRecords = useCallback(
    (
      records: ReadonlyArray<ComposerContextRecord>,
      source: ComposerContextClipboardFragment["source"] | null,
      ...
      for (const candidate of orderedRecords) {
        ...
        switch (record.kind) {
          ...
          case "file": {
            ...
            void importAttachmentRecord(record, localId, source);
            break;
          }
          default:
            break;
  ...
  const importContextFragment = useCallback(
    (fragment: ComposerContextClipboardFragment): ReadonlyMap<string, string> =>
      importContextRecords(fragment.records, fragment.source),
                        ...
                        {...(previewFile.uploadedAttachmentId && previewFile.uploadEnvironmentId
                          ? {
                              asset: {
                                ...
                                ...(previewFile.uploadedAttachmentId.startsWith("pending-")
                                  ? {}
