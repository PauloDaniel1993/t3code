...
export function deriveComposerSendState(options: {
  prompt: string;
  /** Upstream keeps the historic imageCount name for the combined image/file count. */
  imageCount?: number;
  /** Compatibility for the fork's generic-attachment send and retry paths. */
  ...
  const attachmentCount = options.imageCount ?? options.attachmentCount ?? 0;
  return {
    trimmedPrompt,
