      ...
      void (async () => {
        ...
        switch (clicked.value) {
          ...
          case "unsnooze":
            ...
            return;
          case "new-task":
            if (!supportsThreadTasks || isTaskRow) return;
                ...
                <SortableContext items={sortableIds} strategy={sidebarSortingStrategy}>
                  ...
                  >
                    {(() => {
                      ...
                      // Task threads remain structurally owned by their parent:
                      // only the parent participates in upstream's unified DnD
                      // model, while task and native-agent rows follow it inline.
                      ...
                      for (const item of sidebarListItems) {
                        if (item.kind === "thread") {
                          items.push(
                            renderThreadRowWithTasks(threadByKey.get(item.key)!, item.section),
                          );
                          continue;
                        }
                        switch (item.marker) {
