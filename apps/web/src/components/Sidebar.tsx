...
export default function Sidebar() {
  ...
  const {
    ...
    // Pull task children out before ANY lifecycle or pin partitioning. They
    // render immediately after their parent and must never acquire a second
    // identity in the pinned block, shelves, DnD, or keyboard traversal.
  ...
  }, [nowMinute, scopedProjectKeys, serverConfigs, snoozeWakeTick, threadTasksEnabled, threads]);
  ...
  const taskGroupExpandedByThreadId = useUiStateStore((state) => state.taskGroupExpandedByThreadId);
  const setTaskGroupExpanded = useUiStateStore((state) => state.setTaskGroupExpanded);
  const threadLastVisitedAtById = useUiStateStore((state) => state.threadLastVisitedAtById);
  const createTaskMutation = useAtomCommand(threadEnvironment.createTask, { reportFailure: false });
  const cancelTaskMutation = useAtomCommand(threadEnvironment.cancelTask, { reportFailure: false });
  const redeliverTaskMutation = useAtomCommand(threadEnvironment.redeliverTask, {
    reportFailure: false,
  });
  const startTurnMutation = useAtomCommand(threadEnvironment.startTurn, { reportFailure: false });

  const isTaskGroupExpanded = useCallback(
    (
      threadKey: string,
  ...
  // Upstream's Agents panel is the only full agent roster. Sidebar agent rows
  // open that surface instead of restoring the fork's duplicate transcript
  // roster; navigation is only needed when the parent is not already visible.
  const openNativeAgentPanel = useCallback(
    (parentThreadRef: ScopedThreadRef) => {
      if (routeThreadKey !== scopedThreadKey(parentThreadRef)) {
        navigateToThread(parentThreadRef);
      }
      useRightPanelStore.getState().open(parentThreadRef, "agents");
    },
  ...
  const handleCancelTask = useCallback(
    (threadRef: ScopedThreadRef) => {
      const parentThreadId = threads.find(
        (thread) =>
  ...
  const handleRedeliverTask = useCallback(
    (threadRef: ScopedThreadRef) => {
      const parentThreadId = threads.find(
        (thread) =>
          thread.environmentId === threadRef.environmentId && thread.id === threadRef.threadId,
  ...
  const handleThreadContextMenu = useCallback(
    (threadRef: ScopedThreadRef, position: { x: number; y: number }) => {
      void (async () => {
        ...
        const snoozePresets = resolveSnoozePresets(new Date(), timestampFormat);
        // Task identity is durable even when the beta is toggled off. Its
        // lifecycle belongs to the parent, so a child must keep task-safe
        // actions if it temporarily falls back to a top-level row.
        const isTaskRow = thread.parentThreadId != null;
        const clicked = await settlePromise(() =>
          api.contextMenu.show(
            buildThreadActionMenuItems({
              branch: thread.branch ?? null,
              isTask: isTaskRow,
              canCreateTask: supportsThreadTasks,
              isPinned,
              isSettled,
              isSnoozed,
            ...
            >
              <ul ref={attachListAutoAnimateRef} role="list" className="flex flex-col gap-px">
                {(() => {
                  ...
                  // A parent row is followed inline by its task group, so the
                  // parent keeps its own sort position and tasks never enter the
                  // snoozed/settled shelves.
                  const renderThreadRowWithTasks = (
                    thread: EnvironmentThreadShell,
                    section: "pinned" | "active" | "snoozed" | "settled",
                    sortable?: SortablePinnedRowBag,
                  ): ReactNode => {
                    ...
                    return (
                      <Fragment key={`${threadKey}:group`}>
                        ...
                        <SidebarTaskGroup
                          ...
                          onNativeAgentClick={() => openNativeAgentPanel(parentThreadRef)}
                          onContextMenu={handleThreadContextMenu}
                          ...
                          nativeAgentMiniWindow={
                            openPeek?.kind !== "nativeAgent" ? null : (
                              <MiniNativeAgentWindow
                                ...
                                onShowInTranscript={() => {
                                  closeOpenPeek();
                                  openNativeAgentPanel(openPeek.parentThreadRef);
                                }}
                                onOpenAgent={(taskId) => {
                  ...
                  // pins render in place as plain rows). Task children render
                  // after their parent but never register as sortable ids.
                            ...
                            >
                              {orderedPinnedThreads.map((thread) => {
                                ...
                                if (!reorderablePinnedKeys.has(threadKey)) {
                                  return renderThreadRowWithTasks(thread, "pinned");
                                }
                                return (
                                  <SortablePinnedThreadRow key={threadKey} id={threadKey}>
                                    {(bag) => renderThreadRowWithTasks(thread, "pinned", bag)}
                  ...
                  for (const thread of activeThreads) {
                    items.push(renderThreadRowWithTasks(thread, "active"));
                  }
                  ...
                  if (snoozedThreads.length > 0) {
                    ...
                    for (const thread of visibleSnoozedThreads) {
                      items.push(renderThreadRowWithTasks(thread, "snoozed"));
                    }
                  }
                  ...
                  for (const thread of renderedSettledThreads) {
                    items.push(renderThreadRowWithTasks(thread, "settled"));
                  }
                  return items;
                })()}
