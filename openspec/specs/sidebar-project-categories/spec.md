# sidebar-project-categories Specification

## Purpose

User-managed sidebar categories for logical projects: persisting category organization locally, rendering a `category -> project -> thread` tree, collapse without resetting child state, durable project-to-category assignment (with repository-identity fallback/migration), a reversible category lifecycle (create/rename/reorder/hide/unhide/delete), a dedicated `/settings/sidebar` management surface, and a scoped reset that leaves unrelated sidebar behavior intact.
