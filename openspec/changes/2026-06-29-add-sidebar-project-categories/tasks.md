...
- [ ] 1.1 Extend `packages/contracts/src/settings.ts` with `sidebarOrganization` schemas and defaults
- [ ] 1.2 Add contract tests for default decoding and invalid category reference filtering

## 2. Sidebar Organization Domain

- [ ] 2.1 Create `apps/web/src/sidebarOrganization` modules for category definitions, assignment helpers, validation, and full-object settings writes
- [ ] 2.2 Implement durable assignment-key derivation and fallback-to-canonical migration with deterministic `updatedAt` conflict resolution
- [ ] 2.3 Add unit tests for category creation, rename validation, assignment fallback, migration, hide/unhide helpers, and delete-to-`Uncategorized`

## 3. UI State

- [ ] 3.1 Extend `apps/web/src/uiStateStore.ts` with persisted category expansion state
- [ ] 3.2 Add focused tests proving category collapse persistence and reset behavior without clearing project expansion state
...
- [ ] 5.4 Add category selection with inline `New category...` creation to the Add project flow
- [ ] 5.5 Add tests for Add project category selection, inline category creation, and existing-project reassignment
- [ ] 5.6 Add direct sidebar hide action for user-created category headers
- [ ] 5.7 Add tests for sidebar-header hide behavior and `Uncategorized` exclusion
