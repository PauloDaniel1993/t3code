# Tasks â€” handoff-spec-alignment

## 1. Amend plan 23 (same change as code, per AGENTS.md)

- [ ] 1.1 In `.plans/23-provider-thread-handoff.md` Â§UI (~L642-645): replace the ThreadActionsMenu/`MoreHorizontalIcon`/`ArrowRightLeftIcon` requirement with the implemented design â€” composer control-bar button labeled "Hand off" with `GitBranchPlusIcon` (`apps/web/src/components/chat/ChatComposer.tsx:334-355`) plus compact-menu item (`CompactComposerControlsMenu.tsx:90-100`). Note the header menu as a rejected alternative.
- [ ] 1.2 In the same plan Â§Timeline rendering (~L664): change "Imported messages render expanded by default" to "Imported messages render behind a collapsed `N imported messages` fold by default (performance; see `.plans/24`)".
- [ ] 1.3 Verify plan Â§UI dialog behavior says "wait up to 3 seconds" (it does) â€” no plan edit needed for the wait window; the code changes in 3.1.

## 2. Web chain guard

- [ ] 2.1 In `apps/web/src/threadHandoff/handoff.ts`, add a pure helper `hasNativeMessageAfterLastImport(messages): boolean` â€” find the index of the last message with `source === "handoff-import"`; return true if any later message has a native source (`"user" | "provider"`), or if there are no imported messages.
...
- [ ] 2.3 Tests in `apps/web/src/threadHandoff/handoff.test.ts`: (a) thread ending in imported block â†’ blocked with the chain reason; (b) imported block followed by a native user message â†’ allowed; (c) thread with no imports â†’ unaffected.
...
- [ ] 3.2 Add the navigation/timeout test (plan Â§Web tests "Dialog navigates after target appears and toasts on timeout"). Prefer the logic seam: extract the post-dispatch wait/navigate/toast sequence from `ChatView.tsx` into a testable helper in `ChatView.logic.ts` if not already extractable, then in `ChatView.logic.test.ts` use fake timers to assert (a) target appears at t<3000 â†’ navigate called with target thread id; (b) target never appears â†’ toast called, no navigation.
...
- [ ] 4.3 Port the remaining five (`ClaudeTextGeneration.ts`, `CodexTextGeneration.ts`, `CursorTextGeneration.ts`, `GrokTextGeneration.ts`, `OpenCodeTextGeneration.ts`) mechanically â€” each keeps only its transport (CLI/ACP invocation) and passes it to the factory. No behavior change other than the added transient retry.
...
- [ ] 6.3 Run the touched server/web test files (`vp test run` per package); on Windows the server suite fails environmentally (`spawn EFTYPE`) â€” run in WSL/CI.
