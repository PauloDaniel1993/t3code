...
Work entries with a `taskId` SHALL render as distinct task/subagent cards (status badge, task styling) rather than generic tool rows, in both the timeline and side panel.
