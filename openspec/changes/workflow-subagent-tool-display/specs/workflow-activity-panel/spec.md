...
The main conversation area SHALL render a pinned compact workflow activity card — positioned above the message timeline, outside the scrolling message flow, and reflecting the latest turn's activity — containing a step heading, overall step counter, segmented progress strip, clickable step labels, and optional inline worker details.
