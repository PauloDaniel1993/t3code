# Tasks â€” stable-commit-remediation
...
- [ ] 4.2 In `scripts/install-desktop-build.test.ts`: add tests â€” (a) transient lock: rename fails N-1 times then succeeds â†’ completes, backup cleaned; (b) persistent failure: all attempts fail â†’ previous install restored from backup, nonzero exit/thrown error with clear message; (c) attempts/delay constants respected (count facade calls; no real sleeps â€” inject a no-op delay).
