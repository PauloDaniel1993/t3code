...
> **DRAFT — to be refined.** Resolve the open questions in `proposal.md` / `design.md` (storage shape, indicator, cascade semantics) before starting task 1.1.

## 0. Refinement gate

- [ ] 0.1 Confirm open questions 1–6 in `design.md` (storage shape, uncategorized hide, hidden-count indicator, grouped rows, category-hide independence, search reachability) and update proposal/specs/design accordingly

## 1. Contracts and settings schema

- [ ] 1.1 Add `hiddenProjects` map to `SidebarOrganization` in `packages/contracts/src/settings.ts` with tolerant decode (missing/invalid → `{}`)
...
- [ ] 2.4 Extend the fallback→canonical project-key migration in `projectWorkflow.ts` to migrate `hiddenProjects` entries (newer `hiddenAt` wins on conflict)
