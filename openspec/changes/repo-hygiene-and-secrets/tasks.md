# Tasks â€” repo-hygiene-and-secrets
...
- [ ] 1.2 Run `git ls-files .t3-dev/` â€” must output nothing after 1.1.
...
- [ ] 2.2 Start the dev server once (`pnpm dev:server:local`) and confirm it boots â€” `ServerSecretStore.getOrCreateRandom("server-signing-key", 32)` (used from `apps/server/src/auth/SessionStore.ts:402`) regenerates the key. Stop the server afterwards.
...
- [ ] 4.2 `git rm -r .github/skills/ .github/prompts/` (leave the rest of `.github/` untouched â€” verify with `git status` that only `skills/` and `prompts/` under `.github/` are removed).
...
- [ ] 5.3 Run required gates: `vp check` and `vp run typecheck` (both must pass; no code changed so failures indicate an unrelated problem). Note: `vp test` largely cannot run on Windows (`spawn EFTYPE`) â€” run in WSL/CI if needed.
