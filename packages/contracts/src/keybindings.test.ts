...
it.effect("parses keybinding rules", () =>
  Effect.gen(function* () {
    ...
    const parsedProjectBrowserToggle = yield* decode(KeybindingRule, {
      key: "mod+shift+b",
      command: "projectBrowser.toggle",
    });
    assert.strictEqual(parsedProjectBrowserToggle.command, "projectBrowser.toggle");
